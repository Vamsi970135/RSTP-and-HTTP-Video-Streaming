...
def make_btn(parent, text, command, fg=SUCCESS, bg=PANEL):
    """Simple flat button — no canvas, no TclError."""
    b = tk.Button(
...
class RTSPStreamer(tk.Tk):
    ...
    def _build_sidebar(self, parent):
        ...
        self._connect_btn.pack(fill="x", padx=14, pady=4)
        ...
        self._record_btn.pack(fill="x", padx=14, pady=4)
    ...
    def _stop_stream(self):
        ...
        for v in self._info_vars.values():
            v.set("—")
    ...
    def _snapshot(self):
        if not self._running:
            messagebox.showinfo("Snapshot", "No active stream.")
            return
    ...
    def _toggle_record(self):
        if not self._running:
            messagebox.showinfo("Record", "Start a stream first.")
            return
        if self._recording:
            self._recording = False
            if self._writer:
